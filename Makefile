...
release:
	...
	GOPROXY=proxy.golang.org go list -m github.com/go-go-golems/salad@$(shell svu current)
...
SALAD_BINARY=$(shell which salad)
install:
	go build -o ./dist/salad ./cmd/salad && \
		cp ./dist/salad $(SALAD_BINARY)
